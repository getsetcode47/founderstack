...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (error) {
      if (isMissingDealsTable(error)) {
        return NextResponse.json(
          {
            error:
              'The deals table is not available in Supabase yet. Apply the latest migration before editing custom deals.',
          },
          { status: 503 }
        );
      }
    ...
    return NextResponse.json({ error: error?.message || 'Unable to update deal.' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (error) {
      if (isMissingDealsTable(error)) {
        return NextResponse.json(
          {
            error:
              'The deals table is not available in Supabase yet. Apply the latest migration before deleting custom deals.',
          },
          { status: 503 }
        );
      }
    ...
    return NextResponse.json({ error: error?.message || 'Unable to delete deal.' }, { status: 500 });
  }
}
