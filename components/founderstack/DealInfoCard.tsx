...
import { getDealAvatarLabel, getDealHref, getDiscountCtaLabel } from '@/lib/founderstack';
...
export function DealInfoCard({
  ...
  async function ensureUser() {
    if (!user) {
      const redirectTo = pathname ? `${pathname}` : getDealHref(deal);
  ...
  async function createClaim(userId: string) {
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            <Link href={getDealHref(deal)} className="text-xs text-gray-500 transition-colors hover:text-white">
              View
            </Link>
          </div>
