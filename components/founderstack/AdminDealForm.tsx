...
export function AdminDealForm({ deal }: AdminDealFormProps) {
  ...
  async function uploadLogo(file: File) {
    ...
    const base = parts.join('.') || 'logo';
    const path = `${Date.now()}-${base.replace(/\s+/g, '-').toLowerCase()}.${ext}`;
    const { error } = await supabase.storage.from('deal-logos').upload(path, file, {
      cacheControl: '3600',
      upsert: true,
    });
    ...
    const { data } = supabase.storage.from('deal-logos').getPublicUrl(path);
    update('logo_type', 'image');
    update('logo_image_url', data.publicUrl);
      ...
      <div className="space-y-6">
        <section className="rounded-[28px] border border-gray-800 bg-black p-6">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
            ...
            <label className="space-y-2 text-sm text-gray-300">
              ...
              <input
                ...
                className="block h-12 rounded-xl border border-gray-800 bg-white/[0.03] px-4 py-3 text-sm text-gray-300 file:mr-4 file:border-0 file:bg-transparent file:text-white"
              />
          ...
          <div className="mt-4 space-y-4">
            ...
            <Field label="Deal details" required>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button type="button" onClick={() => router.push('/dashboard/deals')} className="inline-flex h-12 items-center justify-center rounded-md border border-gray-800 px-5 text-sm font-medium text-white transition hover:border-gray-600">
            Cancel
          </button>
        </div>
      </div>
